...
- 버전 중간 자리(예: v1.X.Y) 변경 시 운영 DB를 백업(.backup)해 로컬 `data.db`로 교체하고 확인할 것(운영 DB에는 쓰기 금지, 읽기 전용 백업→로컬 교체 방식).
- 배포 시 기본 규칙: v0.0.0의 마지막 자리(패치 버전)를 +1 후 배포. 단, 사용자가 middle/first 버전을 지정해 요청한 경우에는 마지막 자리를 올리지 않는다.
...
- 주간 초기화 알림: 요일별 초기화 게임 목록을 토글형으로 표시, 내일 요일은 파란색 강조.
- 월정액 사용안함: 구성 수정 시 “사용하지 않음” 모드로 설정해 보상/알림/일자 체크 제외 가능.
- 버전 표기: 현재 버전 v.1.2.1.
- 캐시: 서비스워커 캐시 버전 v31으로 갱신해 최신 배포 강제 반영.
